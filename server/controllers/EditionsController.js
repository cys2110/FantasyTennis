...
const getEditionById = async(req, res) => {
    try {
        const { id } = req.params
        const editions = await Edition.findById(id).populate({path: 'winner', select: ['headshot', 'full_name', 'country']}).populate({path: 'finalist', select: ['headshot', 'full_name', 'country']}).populate('tourney').populate({path: 'seeds.player', select: ['headshot', 'full_name', 'country']}).populate({path: 'entry_info.lda.player', select: ['headshot', 'full_name', 'country']}).populate({path: 'entry_info.lls', select: ['headshot', 'full_name', 'country']}).populate({path: 'entry_info.alts', select: ['headshot', 'full_name', 'country']}).populate({path: 'entry_info.wds.player', select: ['headshot', 'full_name', 'country']}).populate({path: 'entry_info.rets.player', select: ['headshot', 'full_name', 'country']}).populate({path: 'entry_info.wos.player', select: ['headshot', 'full_name', 'country']}).populate('tourney')
        if (editions) {
            res.json(editions)
        } else {
